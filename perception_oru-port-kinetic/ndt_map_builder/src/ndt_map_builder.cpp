...
namespace perception_oru{
  ...
  Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor> NDTMapBuilder::addScan(pcl::PointCloud<pcl::PointXYZ> scan, int id)
  {
...
if(doHistogram) {
   ...
   // std::cout << "[ndt_map_builder] " << "T fin " << T.translation().transpose()
             // << " r " << T.rotation().inverse().eulerAngles(0, 1, 2).transpose() << std::endl;
}
      // lx change 0228
...
//             T.setIdentity();
//           }
//         	std::cout<<"T init "<<T.translation().transpose()<<" r "<<T.rotation().inverse().eulerAngles(0,1,2).transpose()<<std::endl;
//         Tndt.setIdentity();
//         cloud3 = perception_oru::transformPointCloud(T,scan);
         ...
         if(isF2F)
           {
             ret = matcherF2F->match(vertices.back().scan,scan,Tndt);
             finalscore = matcherF2F->finalscore;
           }
         else if(isP2F)
           {
             ret = matcherP2F->match(vertices.back().scan,scan,Tndt);
         ...
         //std::cout<<"T fin "<<Tout.translation().transpose()<<" r "<<Tout.rotation().inverse().eulerAngles(0,1,2).transpose()<<std::endl;
